...
$$
Motivation to Vote = \alpha + \beta_1T +\beta_2*Length in Facility + \beta_3 * Ever Voted + \beta_4 * Identifies As Hispanic or Latino + \beta_5 * Identifies As Black + \beta_6* Age +\epsilon
$$
...
prison_cleaned$length_in_this_facility <- ifelse(prison_cleaned$length_in_this_facility == "Over 10 years", 1, 0)
...
mod <- lm(incarceration_impacts_motivation_to_vote ~ length_in_this_facility + ever_voted + identifies_as_hispanic_or_latinx + identifies_as_black + age, data = prison_cleaned)
...
plot_predictions(mod, condition = "length_in_this_facility")
```
