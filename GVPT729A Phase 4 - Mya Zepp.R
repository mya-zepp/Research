...
prison_cleaned<-na.omit(prison_cleaned %>% select(incarceration_impacts_motivation_to_vote,
                                                  length_in_this_facility, identifies_as_black, identifies_as_hispanic_or_latinx, identifies_as_white, ever_voted, party
))

#counts

prison_cleaned |> 
  count(identifies_as_black)

prison_cleaned |> 
  count(identifies_as_white)

prison_cleaned |> 
  count(identifies_as_hispanic_or_latinx)
...
prison_cleaned2 <- prison_cleaned |>
  dplyr::rename(
    "Motivation to Vote" = incarceration_impacts_motivation_to_vote,
    "Time in Facility" = length_in_this_facility,
    "Black" = identifies_as_black,
    "Hispanic/Latinx" = identifies_as_hispanic_or_latinx,
    "Ever Voted" = ever_voted,
    "Party ID" = party
  )
...
tbl <- datasummary(
  (`Motivation to Vote` +
     ...
     `Ever Voted` +
     `Party ID`) ~ Mean + SD + N,
  data = prison_cleaned2,
  title = "Table 1: Incarceration Politics Means",
  output = "gt"
) |>
  fmt_number(
    columns = c("Mean", "SD"),
...
datasummary_crosstab(`Time incarcerated` ~ `Motivation to vote`, data = prison_cleaned, title ="Table 2: Time Incarcerated and Voting Motivation")
...
tbl_2 <- tbl_regression(
  ...
  modify_header(label ~ "**Characteristic**",
                estimate ~ "**Beta**",
                p.value ~ "**p-value**") |>
  modify_caption("**Table 3: Probit Regression of Years in Prison on Vote Motivation**") |>
  as_gt() |>
...
gt_tbl <-
  ...
  tab_header(
    title = md("**Table 4. Summary of Predicted Probabilities**")
  ) |>
  tab_options(table.width = pct(70))
...
gt_tbl_2 <-
  ...
  gt(rowname_col = "Quantity") |>
  tab_header(
    title = md("**Table 5. Predicted Probabilities with 95% CIs (Simulation)**")
  ) |>
  cols_label(
    Mean  = "Mean",
    `2.5%`  = "Lower 95%",
